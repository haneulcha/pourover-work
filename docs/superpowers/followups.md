...
(2026-07-06 스윕: 해소 항목 삭제 — P2-M2는 BR-M1로 통합, P2-M4 scrim 토큰화 완료, P2-M5 주석 추가됨, P2-M6 v2 리디자인에서 섹션 분해됨. 경로를 모노레포 구조로 갱신.)
...
- **파일**: `apps/web/src/features/brewing/BrewingScreen.tsx`
- **내용**: 도메인에 `isComplete(session, now)` 순수 헬퍼가 있으나, 컴포넌트에선 `const done = elapsed >= totalTimeSec` 로 별도 계산 (v2 리디자인 후에도 유지됨).
- **영향**: 진실의 소스 2개. 완료 조건이 바뀌면 두 곳 모두 수정 필요.
- **처리 제안**: `BrewingScreen`에서 도메인 헬퍼 재사용.
...
- **파일**: `apps/web/src/features/wall/WallScreen.tsx`
...
- **내용**: `pourLabel`이 bloom 라벨 없는 푸어를 0-인덱스 그대로 "{idx}차"로 표기 → Kasuya 4:6 첫 푸어가 "지금 부어 · 0차". 기존 동작(구 화면도 동일)이지만 새 화면에서 라벨이 거대해져 어색함이 두드러짐. (구 P2-M2 — Progress rail `0차` edge case — 도 라벨 로직이 도메인 `pourLabel`로 이동하며 이 항목으로 통합.)
- **영향**: 표기 어색함만. 기능 영향 없음.
