...
| `ghost` | 테두리·배경 없음, `text-secondary`, hover→`surface-strong` | 취소. 위험 액션은 className 색 오버라이드가 아니라 추후 danger variant로 (Tailwind 생성 순서상 text-danger가 text-btn-ghost-fg에 밀려 무시됨) |
