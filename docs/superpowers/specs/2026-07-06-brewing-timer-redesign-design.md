...
## Revision v2 (2026-07-06) — 탭 제거, 상시 카운트다운 + 슬림 레일

사용자 피드백: 탭은 (1) 가능하다는 암시가 없고 (2) 결과 예고가 없는 숨은 제스처였다. 근본 해결로 **탭을 제거**하고, 탭이 제공하던 정보(다음까지 남은 시간)를 화면에 상시 표시한다.

### 변경 사항

- **탭 제스처 삭제** — `tapPos` 상태, 전체 화면 버튼, 도메인 `brewPhase`/`BrewPhase`(+테스트) 삭제. 화면은 표시 전용. 필요한 파생값은 기존 `activeStepIdx`/`nextStepIdx`/`leadInCountdown`으로 충분.
- **상태 이원화(부어/기다려) 폐지** — 스텝당 한 레이아웃: 라벨(`지금 · {n}차`) / 큰 숫자(목표 누적 g) / `+{x}g`(마지막 스텝은 `· 마지막`) / **상시 카운트다운** `다음 붓기까지 m:ss` (마지막 스텝은 `완료까지 m:ss`). 모든 숫자 줄에 tabular-nums.
- **드로우다운 상태 삭제** — 붓기 종료 시점을 알 수 없으므로(탭 부재) 마지막 스텝이 완주까지 이어진다. 완료는 기존대로 `elapsed >= totalTimeSec`에서 1회. 조기 완료 경로 없음(부엌 타이머 문법).
- **배경 액센트 = 리드인 전용** — 평소 차분한 캔버스(base), 다음 푸어 5초 전 리드인 창에만 액센트로 물듦(duration-leadin). 토큰 이름을 의미에 맞게 개명: `--color-brewing-state-pour-*` → `--color-brewing-leadin-*`, `--color-brewing-state-wait-*` → `--color-brewing-base-*`.
- **슬림 레일 추가** (접근 2) — 하단 가장자리, 얇은 수평 트랙 + 경과 채움 + 푸어 경계 눈금(점). 라벨 없음, `aria-hidden`(정보는 구석 텍스트·aria-live가 담당). 신규 토큰 `--color-brewing-rail-track/-fill/-tick` (light/dark).
- **길게 눌러 중단 유지** — 컨테이너에 pointer 핸들러(탭과의 충돌·click 억제 로직 불필요해져 단순화). 하단 `길게 눌러 중단` 실제 버튼(키보드/SR 경로) 유지.
- aria-live(경계 전환 안내)는 유지 — 화면이 항상 푸어 목표를 보여주므로 문구 불일치(followups BR-M2)도 해소.
